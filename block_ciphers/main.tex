...
\begin{frame}{The Advanced Encryption Standard}
   ...
   \item \textbf{Standardization (2001):} Officially published as FIPS PUB 197
    ...
    \item The cipher has an SP (\red{substitution}-\green{permutation}) network structure.
    \pause
    \item Block size: $n = 128$ bits, i.e., $\pp \in \{0,1\}^{128} \text{ or } \pp \in \mathbb{F}_2^{128} \text{ or } \pp \in (\mathbb{F}_2^{8})^{16}$
...
\begin{frame}{\tx{AES}-128 Key Schedule}
\begin{columns}[t]
...
\begin{center}
\begin{tikzpicture}[scale=0.4, transform shape, font=\sffamily]
    \begin{scope}[xshift=0cm]
